...
class IssueEngine {
    ...
    getDeepStats() {
        ...
        return this.#issues.reduce((count, issue) => {
            ...
            // priorityCount
            count.priorityCount[issue.priority] =
                (count.priorityCount[issue.priority] || 0) + 1;

            // byStatus
            if (!count.byStatus[issue.status]) {
                count.byStatus[issue.status] = [];
            }
            count.byStatus[issue.status].push(issue);
